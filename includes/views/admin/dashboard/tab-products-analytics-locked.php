<?php
/**
 * Products Analytics — locked stub (FREE edition).
 *
 * @package Cecomwishfw
 */
...
<div class="col-12 mb-3 shadow-sm rounded-4 border border-light-subtle bg-white position-relative overflow-hidden" style="min-height:420px;">
		...
		<div class="table-responsive">
			<table class="table table-sm align-middle mb-0">
				...
				<tbody>
					<?php
					$fake_products = array(
						array( 'Premium Headphones', 245, 82, 112, 18, '16.1%' ),
						array( 'Wireless Keyboard', 198, 67, 88, 12, '13.6%' ),
						array( 'Standing Desk', 176, 54, 71, 9,  '12.7%' ),
						array( 'Ergonomic Chair', 152, 48, 63, 8,  '12.7%' ),
					);
					foreach ( $fake_products as $p ) : ?>
						<tr>
							<td class="fw-medium"><?php echo esc_html( $p[0] ); ?></td>
							<td><?php echo esc_html( (string) $p[1] ); ?></td>
							<td><?php echo esc_html( (string) $p[2] ); ?></td>
							<td><?php echo esc_html( (string) $p[3] ); ?></td>
							<td><?php echo esc_html( (string) $p[4] ); ?></td>
							<td class="text-success"><?php echo esc_html( (string) $p[5] ); ?></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
	...
	<div class="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column align-items-center justify-content-center text-center px-4 rounded-4"
		...
		<p class="fw-bold text-body-emphasis mb-1 fs-5">
			<?php esc_html_e( 'Products Analytics — Pro', 'cecom-wishlist-for-woocommerce' ); ?>
		</p>
		<p class="text-secondary small mb-3" style="max-width:460px;">
			<?php esc_html_e( 'Per-product performance: views, unique users, wishlist adds, purchases, CTR, and one-click campaign creation.', 'cecom-wishlist-for-woocommerce' ); ?>
		</p>
